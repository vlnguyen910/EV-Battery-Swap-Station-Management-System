...
export default function AdminBatteryTransferList() {
  ...
  const filteredRequests = requests.filter(req => {
    ...
    return (
      req.battery_model?.toLowerCase().includes(searchLower) ||
      req.battery_type?.toLowerCase().includes(searchLower) ||
      req.status?.toLowerCase().includes(searchLower) ||
      req.fromStation?.name?.toLowerCase().includes(searchLower) ||
      req.toStation?.name?.toLowerCase().includes(searchLower)
    )
  })
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-lg text-gray-600">Loading transfer requests...</div>
      </div>
    )
  }
            ...
            <div className="flex-1 w-full lg:max-w-full">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search by battery model, type, status, or station name..."
                  value={searchQuery}
                ...
                {currentRequests.length === 0 ? (
                  ...
                  currentRequests.map((req) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-400">
                        {req.fromStation?.name || 'N/A'}
                      </td>
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-400">
                        {req.toStation?.name || 'N/A'}
                      </td>
