...
export default function CreatePackage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setSubmitting(true);
        const newPackage = await packageService.createPackage(values);
        toast.success('Package created successfully');
        navigate(`/admin/packages/${newPackage.package_id}`);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Create Package</h1>
          <p className="text-slate-600 dark:text-slate-400">Add a new package to the system</p>
        </div>
        ...
        <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
          <CardHeader className="border-b border-slate-200 dark:border-slate-800 p-6">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Package Information</h2>
          </CardHeader>
