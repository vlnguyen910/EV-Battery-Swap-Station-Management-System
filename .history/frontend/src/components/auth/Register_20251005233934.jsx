      ...
      <div className="w-1/2 flex items-center justify-center p-8 bg-gradient-to-br from-blue-100 to-green-100">
        <div className="relative">
          ...
          <div className={`relative transition-all duration-1000 ${
            success ? 'animate-bounce' : error ? 'animate-pulse' : ''
          }`}>
            {/* Cabinet Body */}
            <div className="w-64 h-80 bg-gradient-to-b from-gray-300 to-gray-500 rounded-lg shadow-2xl border-4 border-gray-600">
              ...
              <div className="grid grid-cols-2 gap-4 p-4">
                {[1, 2, 3, 4].map((slot) => (
                  <div key={slot} className={`relative transition-all duration-500`}>
                    {/* Battery Slot */}
                    <div className={`w-20 h-24 rounded-lg border-2 flex items-center justify-center ${
                      success ? 'border-green-400 bg-green-100' : 'border-gray-400 bg-white'
                    }`}>
                      {/* Battery Icon */}
                      <div className={`w-12 h-16 rounded transition-all duration-700 ${
                        success ? 'bg-gradient-to-b from-green-400 to-green-600 animate-pulse' : 'bg-gradient-to-b from-gray-300 to-gray-400'
                      }`}>
                        <div className="w-4 h-2 bg-gray-600 mx-auto rounded-t"></div>
                        <div className="flex items-center justify-center h-full">
                          <div className={`text-xs font-bold ${
                            success ? 'text-white' : 'text-gray-600'
                          }`}>⚡</div>
                        </div>
                      </div>
                    </div>
              ...
              {/* Cabinet Door */}
              <div className={`absolute top-16 left-0 w-full h-60 bg-gradient-to-b from-gray-400 to-gray-600 rounded-b-lg border-l-4 border-r-4 border-b-4 border-gray-600 transition-all duration-1000 transform origin-left ${
                success ? 'rotate-y-90 opacity-50' : error ? 'animate-shake' : ''
              }`}>
          ...
          {/* Person Character */}
          <div className={`absolute -right-24 top-1/2 transform -translate-y-1/2 transition-all duration-1000 ${
            loading ? 'animate-bounce' : success ? 'scale-110' : error ? 'opacity-70' : ''
          }`}>
            {/* Person Body */}
            <div className="relative">
              {/* Head */}
              <div className={`w-12 h-12 rounded-full mx-auto mb-2 ${
                success ? 'bg-gradient-to-b from-yellow-300 to-yellow-400' : error ? 'bg-gradient-to-b from-gray-400 to-gray-500' : 'bg-gradient-to-b from-yellow-200 to-yellow-300'
              }`}>
                {/* Face */}
              ...
              {/* Body */}
              <div className={`w-8 h-16 mx-auto rounded-t-full ${
                success ? 'bg-gradient-to-b from-blue-400 to-blue-600' : 'bg-gradient-to-b from-blue-300 to-blue-500'
              }`}></div>
              
              {/* Arms */}
              <div className={`absolute top-14 -left-2 w-6 h-2 rounded-full transform transition-all duration-500 ${
                success ? 'bg-yellow-300 rotate-45' : 'bg-yellow-200 rotate-12'
              }`}></div>
              <div className={`absolute top-14 -right-2 w-6 h-2 rounded-full transform transition-all duration-500 ${
                success ? 'bg-yellow-300 -rotate-45' : 'bg-yellow-200 -rotate-12'
              }`}></div>
          ...
          <div className="absolute -bottom-16 left-1/2 transform -translate-x-1/2 text-center">
            <p className={`font-semibold text-lg transition-all duration-300 ${
              success ? 'text-green-600' : error ? 'text-red-600' : loading ? 'text-blue-600' : 'text-gray-600'
            }`}>
